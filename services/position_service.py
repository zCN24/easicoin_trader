...
class PositionService:
    ...
    async def get_positions(self, symbol: Optional[str] = None) -> list[Position]:
        ...
        data = await self._client.get("position/list", params=params)
    ...
    async def set_margin_mode(self, symbol: str, mode: str) -> Position:
        ...
        if mode not in {"cross", "isolated"}:
            raise ValueError("mode 仅支持 cross 或 isolated")

        data = await self._client.post(
            "position/margin-mode",
            data={"symbol": symbol, "mode": mode},
        )
        return Position.model_validate(data)
    ...
    async def close_position(self, symbol: str, size: Optional[float] = None) -> Position:
        ...
        if size is not None:
            payload["size"] = size
        data = await self._client.post("position/close", data=payload)
        return Position.model_validate(data)
    ...
    async def set_tp_sl(
        ...
        data = await self._client.post("position/tp-sl", data=payload)
        return Position.model_validate(data)
    ...
    async def adjust_margin(self, symbol: str, amount: float, action: str) -> Position:
        ...
        data = await self._client.post(
            "position/adjust-margin",
            data={"symbol": symbol, "amount": amount, "action": action},
        )
        return Position.model_validate(data)
